...
async def test_overview_counts_proposals_by_status(client, registered, make_proposal_api):
    await make_proposal_api(client, registered.headers, project_name="P1")
    await make_proposal_api(client, registered.headers, project_name="P2")
    resp = await client.get(f"{API}/analytics/overview", headers=registered.headers)
    data = resp.json()
...
async def test_proposal_analytics_detail(client, registered, make_proposal_api):
...
async def test_proposal_analytics_cross_agency_404(client, registered, second_agency, make_proposal_api):
    p = await make_proposal_api(client, registered.headers)
...
async def test_proposal_visitors_empty(client, registered, make_proposal_api):
    p = await make_proposal_api(client, registered.headers)
    resp = await client.get(
